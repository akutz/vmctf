...
data "ignition_config" "master_config" {
  ...
  files = [
    ...
    "${data.ignition_file.master_k8s_admin_kubeconfig.id}",
    "${data.ignition_file.master_tls_kube_apiserver_crt.id}",
    "${data.ignition_file.master_tls_kube_apiserver_key.id}",
    "${data.ignition_file.master_tls_k8s_service_accounts_key.id}",
    "${data.ignition_file.master_tls_k8s_service_accounts_crt.id}",
  ]
  ...
  systemd = [
    ...
    "${data.ignition_systemd_unit.master_kube_online_target.id}",
